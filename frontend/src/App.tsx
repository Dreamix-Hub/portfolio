...
const ContactButton = () => (
  <button
    type="button"
    className="group relative rounded-full px-8 py-3 text-xs font-medium uppercase tracking-widest text-white outline outline-2 outline-white outline-offset-[-3px] transition duration-200 hover:opacity-90 sm:px-10 sm:py-3.5 sm:text-sm md:px-12 md:py-4 md:text-base"
    style={{
...
const LiveProjectButton = () => (
  <button
    type="button"
    className="rounded-full border-2 border-[#D7E2EA] px-8 py-3 text-sm font-medium uppercase tracking-widest text-[#D7E2EA] transition duration-200 hover:bg-[#D7E2EA]/10 sm:px-10 sm:py-3.5 sm:text-base"
  >
    Live Project
  </button>
)
...
const AnimatedCharacter = ({
  ...
  return (
    <span className="relative inline-block">
      ...
      <motion.span style={{ opacity }} className="absolute inset-0 left-0 top-0">
        {displayChar}
      </motion.span>
    </span>
  )
...
const AnimatedText = ({ text }: { text: string }) => {
  ...
  return (
    <p
      ref={paragraphRef}
      className="relative mx-auto max-w-[560px] text-center text-[#D7E2EA] font-medium leading-relaxed"
  ...
  <section className="relative flex h-screen flex-col overflow-x-clip">
    ...
    >
      {['About', 'Price', 'Projects', 'Contact'].map((item) => (
        <a key={item} href="#" className="transition duration-200 hover:opacity-70">
          {item}
        </a>
      ))}
    </FadeIn>

    <div className="relative z-20 mt-auto mb-auto overflow-hidden">
      <FadeIn
        as="h1"
        delay={0.15}
        y={40}
    ...
    <div className="relative z-20 flex items-end justify-between px-6 pb-7 sm:pb-8 md:px-10 md:pb-10">
      <FadeIn
        ...
        delay={0.35}
        y={20}
        className="max-w-[160px] font-light uppercase tracking-wide text-[#D7E2EA] sm:max-w-[220px] md:max-w-[260px]"
        style={{ fontSize: 'clamp(0.75rem, 1.4vw, 1.5rem)', lineHeight: 1.3 }}
      >
...
const MarqueeRow = ({ images, x }: { images: string[]; x: number }) => (
  <div
  ...
  return (
    <section ref={sectionRef} className="bg-[#0C0C0C] px-5 pb-10 pt-24 sm:px-8 sm:pt-32 md:px-10 md:pt-40">
      <div className="space-y-3 overflow-hidden">
        <MarqueeRow images={[...rowOne, ...rowOne, ...rowOne]} x={offset - 200} />
        <MarqueeRow images={[...rowTwo, ...rowTwo, ...rowTwo]} x={-(offset - 200)} />
      </div>
    </section>
  )
}
...
const AboutSection = () => (
  <section className="relative flex min-h-screen items-center justify-center overflow-hidden px-5 py-20 sm:px-8 md:px-10">
    <FadeIn
      delay={0.1}
      duration={0.9}
      x={-80}
    ...
    <div className="relative z-10 flex w-full flex-col items-center gap-10 sm:gap-14 md:gap-16">
      <FadeIn as="h2" delay={0} y={40} className="hero-heading text-center font-black uppercase tracking-tight leading-none" style={{ fontSize: 'clamp(3rem, 12vw, 160px)' }}>
        About me
      </FadeIn>
      <div className="flex flex-col items-center gap-16 sm:gap-20 md:gap-24">
        <AnimatedText text={animatedParagraph} />
        <ContactButton />
      </div>
    </div>
  </section>
  ...
  <section className="rounded-t-[40px] bg-white px-5 py-20 sm:rounded-t-[50px] sm:px-8 sm:py-24 md:rounded-t-[60px] md:px-10 md:py-32">
    ...
    <div className="mx-auto max-w-5xl">
      {services.map((service, index) => (
        ...
        >
          <div className="font-black leading-none text-[#0C0C0C]" style={{ fontSize: 'clamp(3rem, 10vw, 140px)' }}>
            {service.number}
          </div>
      ...
      >
        <div className="mb-5 flex flex-col gap-4 sm:mb-8 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-end gap-4 sm:gap-6">
            ...
            <div className="pb-2">
              <p className="text-sm font-light uppercase tracking-widest text-[#D7E2EA]/70">{project.category}</p>
              <h3 className="text-xl font-medium uppercase text-[#D7E2EA] sm:text-2xl md:text-3xl">{project.name}</h3>
            </div>
          </div>
          <LiveProjectButton />
        </div>
...
const ProjectsSection = () => (
  <section className="relative z-10 -mt-10 rounded-t-[40px] bg-[#0C0C0C] px-5 py-20 sm:-mt-12 sm:rounded-t-[50px] sm:px-8 md:-mt-14 md:rounded-t-[60px] md:px-10 md:py-24">
    <h2
